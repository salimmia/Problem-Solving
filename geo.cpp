...
struct Point {
    ...
    Point operator * (Tf u) { return Point(x * u, y * u); } /// add const
    Point operator / (const Tf u) const { return Point(x / u, y / u); } /// remove const
...
struct line {
    Point a, b;
    Point v; Tf c;
    line(Point v, Tf c) : v(v), c(c) {} // From direction vector v and offset c
    line(Tf a, Tf b, Tf c) : v( {b, -a}), c(c) {} // From equation ax+by=c
    line(Point p, Point q) : v(q - p), a(p), b(q), c(cross(v, p)) {} // From points P and Q
...
Point circumCenter(Point a, Point b, Point c) { /// OK
    ...
    return a + rotate90(b * sqLength(c) - c * sqLength(b)) / cross(b, c) / 2;
}
