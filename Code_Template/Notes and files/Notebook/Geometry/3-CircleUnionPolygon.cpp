    ...
    for(int j=0; j<n; j++){if(i != j){
      ...
      if(sign(tmp.first)<= 0 && sign(tmp.second)<= 0){
       seg.push_back(pair<double,double> 
        (2*PI+tmp.first,2*PI+tmp.second));}
...
vector<PT>convex_hull(vector<PT>&p){
 ...
 sort(v.begin(),v.end());vector<PT>up,dn;
 for(auto&p:v){
...
const int top){//O(log n)///not tested
...
Tf width(vector<PT>&p){
 ...
 while(i<n){
  while(cross(p[(i+1)% n]-p[i],
   p[(j+1)% n]-p[j])>= 0)j=(j+1)% n;
  ans=min(ans,dist_from_point_to_line
   (p[i],p[(i+1)% n],p[j]));
  i++;}return ans;
}
//minimum perimeter
...
while(step--){if(cross(u[(i+1)% n]-u[i],
 v[(j+1)% m]-v[j])>= 0)j=(j+1)% m;
